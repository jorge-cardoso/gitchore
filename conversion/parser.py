...
if __name__ == '__main__':
    ...
    parser = Parser(filename=os.path.join(SAMPLE_DATA_DIR, 'ultra_scale_aiops.md'))

    project = {}

    section = parser.get_element('h1', startswith='Overview')
    if not section:
        print('Unable to find section:', 'Overview')
    else:
        project['Overview'] = parser.get_key_values(section[-1])

    # section = parser.get_element('h1', startswith='Description')
    # if not section:
    #     print('Unable to find section:', 'Description')
    # else:
    #     project['Description'] = parser.get_key_values(section[-1])
    #
    # section = parser.get_element('h1', startswith='Results')
    # if not section:
    #     print('Unable to find section:', 'Results')
    # else:
    #     project['Results'] = parser.get_key_values(section[-1])
    #
    # section = parser.get_element('h2', startswith='Tasks')
    # if not section:
    #     print('Unable to find section:', 'Tasks')
    # else:
    #     project['Tasks'] = parser.get_key_value(section[-1])
    #
    # section = parser.get_element('h2', startswith='Sprints')
    # if not section:
    #     print('Unable to find section:', 'Sprints')
    # else:
    #     project['Sprints'] = parser.get_sprints(section[-1])
